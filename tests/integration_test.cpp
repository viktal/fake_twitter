...
void onfail(std::exception_ptr exceptionPtr) {
    GTEST_FAIL();
}
...
std::vector<model::User> make_users(Http::Client& client, int N) {
    ...
    // create random users
    const std::string url = "http://" + ADDRESS + ":" + std::to_string(PORT) + "/0.0/users/create?";
    for (int i = 0; i < N; i++) {
        auto userToInsert = fake::user::object();
        auto posturl = url + "username=" + userToInsert.username + "&name=" + userToInsert.name;
        auto response = client.post(posturl)
                              .send();
        response.then(
...
void update_users(Http::Client& client, std::vector<model::User>& users) {
    std::vector<Async::Promise<Http::Response>> responses;
    const std::string url = "http://" + ADDRESS + ":" + std::to_string(PORT) + "/0.0/users/update?";
...
void select_users(Http::Client& client, const std::vector<model::User>& users, bool expect_fail) {
    std::vector<Async::Promise<Http::Response>> responses;

    const std::string url = "http://" + ADDRESS + ":" + std::to_string(PORT) + "/0.0/users/show?";
...
void drop(Http::Client& client, const std::vector<model::User>& users, bool expect_fail) {
    std::vector<Async::Promise<Http::Response>> responses;
    const std::string url = "http://" + ADDRESS + ":" + std::to_string(PORT) + "/0.0/users/drop?";
